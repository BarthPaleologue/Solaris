...
export class Solaris {
    ...
    constructor(engine, quality = "high") {
        ...
        this.zooming = false; // Flag pour zoomer sur les astres
        this.changement = false; // Flag pour se déplacer vers un autre astre
        this.targeting = false; // Flag pour lock la caméra pendant les déplacements
    ...
    updateCameraPositions() {
        if (this.scene.activeCamera instanceof BABYLON.ArcRotateCamera) {
            ...
            if (BABYLON.Vector3.DistanceSquared(this.scene.activeCamera.position, this.currentTarget.mesh.absolutePosition) < Math.pow(this.destinationRadius, 2)) {
                this.endTravel();
            }
            else {
