...
function randomStarColor() {
    if (randBool(50))
        return new BABYLON.Color4(rand(.1, .2), rand(.5, .8), 1, 1);
    else
        return new BABYLON.Color4(rand(.7, 253 / 255), rand(.8, 184 / 255), 1, 1);
}
class Galaxy {
    constructor(_scene, _nbStars, _nbArms = 5, _armLength = 10, _armOffsetMax = 30, _rotationFactor = .05, _largeCoeff = 3, _dispersion = 5) {
        this.scene = _scene;
        ...
        this.stars.addPoints(this.nbStars, (particle, i) => {
            particle.position = this.starsPositions[i];
            particle.color = randomStarColor();
        });
        this.stars.updateParticle = (particle) => particle.position = this.starsPositions[particle.idx];
        ...
        this.gazClouds.updateFunction = (particles) => {
            for (let i in particles)
                particles[i].position = this.gazCloudsPositions[i];
        };
    ...
    initPositions(type) {
        let positions = new Array(this.nbStars);
        for (let i = 0; i < this.nbStars; i++) {
            let angularDifference = (2 * Math.PI) / this.nbArms;
            let distance = Math.pow(rand(1, this.armLength), 2); /// Initialisation de la distance au centre
            let armOffset = (Math.pow((rand(-1, 1) * this.armOffsetMax / distance), 2)) * this.largeCoeff; /// Définition du décalage en fonction de la distance
...
export function createGalaxy(nb) {
    ...
    let pipeline = new BABYLON.DefaultRenderingPipeline("pipeline", // The name of the pipeline
    true, /// HDR
    ...
    new Slider("precision", document.getElementById("precision"), 1, 50, 20, (val) => {
        camAstras.wheelPrecision = val / 10;
        camAstrasd.wheelPrecision = val / 10;
    });
    new Slider("speed", document.getElementById("speed"), 1, 50, 10, (val) => {
        camfree.speed = val;
        camfreed.speed = val;
    });
    new Slider("sensibility", document.getElementById("sensifree"), 1, 20, 5, (val) => {
    ...
    let armslider = new Slider("nbArms", document.getElementById("arms"), 2, 10, galaxy.nbArms, (val) => {
        galaxy.nbArms = val;
        galaxy.update();
    });
    let densityslider = new Slider("density", document.getElementById("denstar"), 1, 30, Math.round(10 / galaxy.density), (val) => {
        galaxy.density = 10 / val;
        galaxy.update();
    });
    let enrouslider = new Slider("enroulement", document.getElementById("enroulement"), 1, 300, galaxy.rotationFactor * 2000, (val) => {
        galaxy.rotationFactor = val / 2000;
        galaxy.update();
    });
    let largeslider = new Slider("largeur", document.getElementById("largeur"), 1, 70, galaxy.largeCoeff * 10, (val) => {
        galaxy.largeCoeff = val / 10;
        galaxy.update();
    });
    let longueurslider = new Slider("length", document.getElementById("longueur"), 6, 16, galaxy.armLength, (val) => {
        galaxy.armLength = val;
        galaxy.update();
    });
    let dispslider = new Slider("dispersion", document.getElementById("disp"), 0, 10, galaxy.dispersion, (val) => {
