export interface CoronaSettings {
    planetRadius: number,
...
export class StarCorona extends BABYLON.PostProcess {
    ...
    constructor(name: string, planet: BABYLON.Mesh, planetRadius: number, atmosphereRadius: number, sun: BABYLON.Mesh | BABYLON.PointLight, camera: BABYLON.Camera, scene: BABYLON.Scene) {
        ...
        this.onBeforeRender = (effect: BABYLON.Effect) => {
            ...
            effect.setMatrix("projection", this.camera.getProjectionMatrix(true));
            effect.setMatrix("view", this.camera.getViewMatrix(true));
