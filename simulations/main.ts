...
export function initSolaris(pathToData: string, quality: string = "high") {
    ...
    new Slider("speed", document.getElementById("speed"), 1, 100, 50, (val: number) => {
        system.freeCameraSpeed = (val * 0.001) ** 2 * system.distanceScalingFactor;
    });
    ...
    document.onkeydown = e => { // gestion de la vitesse du temps avec les touches + et -
        ...
        if (e.keyCode == 107 && system.timeUnit < 100 ** system.powerTime * system.timeSpeedFactor) { /// +
    ...
    document.addEventListener("dateChange", e => {
        let newDate = (<CustomEvent>e).detail;
    ...
    document.getElementById("date").addEventListener("click", () => { // on click sur la date
        document.getElementById("dateSelectorContainer").classList.toggle("hiddenDateSelector"); // l'interface de voyage temporel s'ouvre
    });
    document.getElementById("closeDateSelector").addEventListener("click", () => { // on click sur la date
        document.getElementById("dateSelectorContainer").classList.toggle("hiddenDateSelector"); // l'interface de voyage temporel s'ouvre
    });
    document.getElementById("changeDateButton").addEventListener("click", () => { // on click sur la date
        let oldDate = new Date(document.getElementById("date").textContent.replace(/\//g, "-")); // remplace les / par des tirets
        let newDate = new Date((<HTMLInputElement>document.getElementById("dateSelector")).value);
