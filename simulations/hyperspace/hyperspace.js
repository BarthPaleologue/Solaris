...
class Hyperspace {
    constructor(canvas) {
        ...
        this.loadingCompleteEvent = new CustomEvent("loadingComplete");
    ...
    update() {
        if (this.isFTL) {
            if (this.speed < 0.2)
                this.speed += 0.0005 * this.acceleration;
            ...
            if (this.speed > 0.0001)
                this.speed -= 0.003 * this.acceleration;
            if (this.speed <= 0)
                this.speed += 0.001;
            if (this.camera1.fov > 0.4)
                this.camera1.fov -= 0.1 * this.acceleration;
            if (this.camera2.fov > 0.4)
                this.camera2.fov -= 0.1 * this.acceleration;
        }
        this.starField1.position.z -= this.speed * this.speedFactor;
        this.starField2.position.z -= this.speed * this.speedFactor;
        if (this.starField1.position.z < -450)
            this.starField1.position.z = 150;
        if (this.starField2.position.z < -450)
            this.starField2.position.z = 150;
        document.dispatchEvent(this.updateEvent);
    }
}
export function createHyperspace(canvas) {
