...
export function initHyperspace() {
    ...
    new Slider("speed", document.getElementById("speed"), 1, 20, hyperspace.speedFactor, (val: number) => {
        hyperspace.speedFactor = val;
    });

    let fovSlider = new Slider("fov", document.getElementById("fov"), 150, 314, hyperspace.fovMax * 100, (val: number) => {
        hyperspace.fovMax = val / 100;
    });

    new Slider("trans", document.getElementById("trans"), 1, 20, hyperspace.acceleration * 10, (val: number) => {
